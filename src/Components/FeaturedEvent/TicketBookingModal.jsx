...
import { X, User, Mail, Phone, Ticket, Calendar, MapPin, Tag } from "lucide-react";
import { toast } from "react-toastify";
import axios from "axios";

const baseUrl = import.meta.env.VITE_API_URL;

const TicketBookingModal = ({ isOpen, onClose, eventData }) => {
  ...
  // Fetch ticket formats when modal opens
  useEffect(() => {
    if (isOpen && eventData?.ticketFormats?.length > 0) {
      fetchTicketFormats();
    }
  }, [isOpen, eventData]);
  ...
  const fetchTicketFormats = async () => {
    try {
      setLoading(true);
      const formatPromises = eventData.ticketFormats.map((formatId) =>
        axios.get(`${baseUrl}/api/ticketFormat/${formatId}`, {
          headers: { Authorization: authToken },
        })
      );
      const responses = await Promise.all(formatPromises);
      const formats = responses.map((res) => res.data);
      setTicketFormats(formats);
      if (formats.length > 0) {
        setSelectedTicketFormat(formats[0]);
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-pink-500 to-pink-600 text-white">
          <div>
            <h2 className="text-2xl font-bold">Book Tickets</h2>
            <p className="text-pink-100 text-sm mt-1">{eventData?.name}</p>
          </div>
          <button
            onClick={onClose}
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                          selectedTicketFormat?._id === format._id
