...
function EventHeading({ heading, startDate, endDate, by }) {
  return (
    <div className=" m-1 rounded-xl p-2 border-2 pb-0 pl-0 pr-0" style={{boxShadow:"2px 2px 10px white"}}>
      <h1 className="p-2 text-3xl font-semibold font-serif">{heading}</h1>

      <p className="font-semibold flex gap-3 p-2 pb-1">
        <MdEvent className="text-[#ff2459] text-xl relative top-1" />
        {startDate} - {endDate}
      </p>
      <p className="p-2 text-lg flex gap-10">
        By {by}{" "}
        <Button
          text={"live music"}
          variant={"primary"}
      ...
      <div className="flex rounded-b-lg font-semibold text-lg p-2 gap-2 bg-[#ffa7bc]">
        Share Event
