...
import { FaUsers, FaPlus, FaTrash, FaSearch, FaEye, FaPencilAlt, FaFilter, FaTimes, FaExclamationTriangle, FaCheckCircle, FaMapMarkerAlt, FaStar, FaCalendarAlt } from 'react-icons/fa';
import { getAllOrganizers } from '../../redux/actions/master/Organizer/getOrganizers';
import { enableOrganizer, disableOrganizer, deleteOrganizer as deleteOrganizerAction, bulkEnableOrganizers, bulkDisableOrganizers, bulkDeleteOrganizers } from '../../redux/actions/master/Organizer/OrganizerOps';
...
const AdminOrganizers = ({ onNavigateToViewOrganizer, onNavigateToEditOrganizer, onNavigateToAddOrganizer }) => {
    ...
    // Fetch organizers function
    const fetchOrganizers = async (page = 1, search = '', categoryFilter = '', cityFilter = '', stateFilter = '') => {
        setLoading(true);
        try {
            const params = {
                page,
            ...
            const result = await dispatch(getAllOrganizers(params));
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            fetchOrganizers(1, searchTerm, filters.categories, filters.city, filters.state);
        }, 500);
    ...
    const handleSelect = (id) => {
        setSelectedOrganizers(prev =>
            prev.includes(id) ? prev.filter(organizerId => organizerId !== id) : [...prev, id]
        );
    };
    ...
    const handleSelectAll = () => {
        if (selectedOrganizers.length === organizers.length) {
            ...
            setSelectedOrganizers(organizers.map(organizer => organizer._id));
        }
    };
    ...
    // Single operations
    const handleSingleEnable = async (organizerId) => {
        const organizer = organizers.find(o => o._id === organizerId);
        showConfirmation(
            'Enable Organizer',
            `Are you sure you want to enable "${organizer?.name}"? This will make them active.`,
            'Enable Organizer',
            'Cancel',
            'success',
            async () => {
                try {
                    await dispatch(enableOrganizer(organizerId));
                    fetchOrganizers(pagination.currentPage, searchTerm, filters.categories, filters.city, filters.state);
                } catch (e) { }
            }
        );
    };

    const handleSingleDisable = async (organizerId) => {
        const organizer = organizers.find(o => o._id === organizerId);
        showConfirmation(
            'Disable Organizer',
            `Are you sure you want to disable "${organizer?.name}"? This will make them inactive.`,
            'Disable Organizer',
            'Cancel',
            'warning',
            async () => {
                try {
                    await dispatch(disableOrganizer(organizerId));
                    fetchOrganizers(pagination.currentPage, searchTerm, filters.categories, filters.city, filters.state);
                } catch (e) { }
            }
        );
    ...
    const handleDeleteSingle = async (organizerId) => {
        const organizer = organizers.find(o => o._id === organizerId);
        showConfirmation(
            'Delete Organizer',
            ...
            async () => {
                try {
                    await dispatch(deleteOrganizerAction(organizerId));
    ...
    const handleBulkEnable = async () => {
        if (selectedOrganizers.length === 0) return alert('Please select organizers to enable.');
        showConfirmation(
            'Enable Organizers',
            `Are you sure you want to enable ${selectedOrganizers.length} selected organizer(s)?`,
            'Enable Organizers',
            'Cancel',
            'success',
            async () => {
                try {
                    await dispatch(bulkEnableOrganizers(selectedOrganizers));
                    setSelectedOrganizers([]);
                    fetchOrganizers(pagination.currentPage, searchTerm, filters.categories, filters.city, filters.state);
                } catch (e) { }
            }
    ...
    const handleBulkDisable = async () => {
        if (selectedOrganizers.length === 0) return alert('Please select organizers to disable.');
        showConfirmation(
            'Disable Organizers',
            `Are you sure you want to disable ${selectedOrganizers.length} selected organizer(s)?`,
            'Disable Organizers',
            'Cancel',
            'warning',
            async () => {
                try {
                    await dispatch(bulkDisableOrganizers(selectedOrganizers));
                    setSelectedOrganizers([]);
                    fetchOrganizers(pagination.currentPage, searchTerm, filters.categories, filters.city, filters.state);
                } catch (e) { }
            }
    ...
    const handleBulkDelete = async () => {
        if (selectedOrganizers.length === 0) return alert('Please select organizers to delete.');
        showConfirmation(
            'Delete Organizers',
            `Are you sure you want to delete ${selectedOrganizers.length} selected organizer(s)? This action cannot be undone.`,
            'Delete Organizers',
            'Cancel',
            'danger',
            async () => {
                try {
                    await dispatch(bulkDeleteOrganizers(selectedOrganizers));
                    setSelectedOrganizers([]);
                    fetchOrganizers(pagination.currentPage, searchTerm, filters.categories, filters.city, filters.state);
                } catch (e) { }
            }
    ...
    const handlePageChange = (page) => {
        fetchOrganizers(page, searchTerm, filters.categories, filters.city, filters.state);
    };
            ...
            <div className="space-y-5">
                {loading ? (
                    <div className="text-center py-16 px-6 bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <h3 className="text-xl font-semibold text-gray-700">Loading Organizers...</h3>
                    </div>
                ) : organizers.length > 0 ? (
                    <>
                        {/* Select All Checkbox */}
                        <div className="flex items-center space-x-3 p-4 bg-white rounded-lg shadow-sm border border-gray-200">
                            <input
                                type="checkbox"
                                checked={selectedOrganizers.length === organizers.length && organizers.length > 0}
                                onChange={handleSelectAll}
                                className="h-5 w-5 rounded text-indigo-600 border-gray-300 focus:ring-indigo-500"
                            />
                            <span className="text-sm font-medium text-gray-700">
                                Select All ({selectedOrganizers.length} of {organizers.length} selected)
                            </span>
                        </div>

                        {organizers.map(organizer => (
                            <div key={organizer._id} className={`bg-white rounded-lg shadow-sm border transition-all duration-300 hover:shadow-lg ${selectedOrganizers.includes(organizer._id) ? 'border-indigo-500 ring-2 ring-indigo-200' : 'border-gray-200 hover:border-indigo-300'}`}>
                                <div className="p-4 border-b border-gray-200 flex justify-between items-center flex-wrap gap-2">
                                    <div className="flex items-center space-x-3">
                                        <input
                                            ...
                                            onChange={() => handleSelect(organizer._id)}
                                            className="h-5 w-5 rounded text-indigo-600 border-gray-300 focus:ring-indigo-500"
                                        />
                                        <div>
                                            <h3 className="font-bold text-lg text-indigo-700">{organizer.name}</h3>
                                            <p className="text-sm text-gray-500">Organizer ID: <span className="font-medium text-gray-600">{organizer._id}</span></p>
                                        </div>
                                    </div>
                                ...
                                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-5">
                                    <DetailItem icon={<FaMapMarkerAlt size={14} />} label="Location">{organizer.city}, {organizer.state}</DetailItem>
                                    {/* <DetailItem icon={<FaStar size={14} />} label="Rating">{formatRating(organizer.rating)}</DetailItem> */}
                                    <DetailItem icon={<FaUsers size={14} />} label="Categories">{formatCategories(organizer.categories)}</DetailItem>
                    ...
                    <div className="text-center py-16 px-6 bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <h3 className="text-xl font-semibold text-gray-700">No Organizers Found</h3>
                        <p className="text-gray-500 mt-2">
                            {searchTerm || filters.categories || filters.city || filters.state
                                ? `Your search for "${searchTerm}" with filters did not match any organizers.`
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                            ...
                            <div className="text-sm text-gray-600">
                                Showing {((pagination.currentPage - 1) * pagination.limit) + 1} to {Math.min(pagination.currentPage * pagination.limit, pagination.totalOrganizers)} of {pagination.totalOrganizers} organizers
                            </div>
