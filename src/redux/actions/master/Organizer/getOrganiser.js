...
// let api = "https://dev.eventsnode.com/api/organizer";
  ...
  switch (filter) {
    case "title asc":
      if (category) {
        ...
        // api = `https://dev.eventsnode.com/api/organizer/filter?categories=${encodeURIComponent(
        //   category
        // )}&page=${page}&limit=9&sortOrder=asc`;
      } else {
        api = `${Organizer.getOrganizerByFilter}page=${page}&limit=9&sortOrder=asc&sortBy=name`;
        // api = `https://dev.eventsnode.com/api/organizer/filter?page=${page}&limit=9&sortOrder=asc`;
      }
    ...
    case "title desc":
      ...
      // api = `https://dev.eventsnode.com/api/organizer/filter?page=${page}&limit=8&sortOrder=desc`;
      break;
    ...
    case "alphabetical":
      if (category) {
        ...
        api = `${Organizer.getOrganizerByFilter}page=${page}&limit=9&sortOrder=asc&sortBy=name`;
      // api = `https://dev.eventsnode.com/api/organizer/filter?page=${page}&limit=8&sortOrder=asc`;
      break;
    ...
    default:
      ...
      // api = ` https://dev.eventsnode.com/api/organizer/filter?page=${page}&limit=9`;
      break;
  }
