...
let api = "http://localhost:5000/api/organizer/filter";
...
export const getOrganizer = (setLoader, filter, page = 1, limit = 10) => {
  return async (dispatch) => {
    ...
    switch (filter) {
      case "title asc":
        api = `http://localhost:5000/api/organizer/filter?page=${page}&limit=${limit}&sortOrder=asc`;
        break;

      case "title desc":
        api = `http://localhost:5000/api/organizer/filter?${page}&limit=${limit}&sortOrder=desc`;
        break;

      case "alphabetical":
        api = `http://localhost:5000/api/organizer/filter?${page}&limit=${limit}&sortOrder=asc`;
        break;

      default:
        api = " http://localhost:5000/api/organizer/filter?page=1&limit=10";
        break;
    }
